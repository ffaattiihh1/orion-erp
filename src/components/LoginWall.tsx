...
import { ShieldCheck, Lock, ArrowRight, UserCheck, Smartphone, KeyRound, Mail, CheckCircle2, AlertCircle, X } from 'lucide-react';

export default function LoginWall() {
  const { loginWithCredentials, sendPasswordReset, users, setCurrentUser } = useApp();
  ...
  const handleQuickDemo = (userCode: string) => {
    const user = users.find(u => u.username === userCode || (userCode === 'admin' && u.role === 'admin'));
    if (user) {
      setCurrentUser(user);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden text-slate-100">
      ...
      {/* Main Card */}
      <div className="w-full max-w-md bg-slate-900/90 border border-slate-800 rounded-3xl shadow-2xl backdrop-blur-xl p-8 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-7">
          ...
          <div className="mt-3 flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[11px] font-medium">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Örn: fatih.sakar veya fatihsakar@orionarastirma.com"
          ...
          >
            <span>Giriş Yap (Oturumu Koru)</span>
        ...
        <p className="mt-5 text-center text-[11px] text-slate-500">
          Cihazınızda oturum açıldıktan sonra otomatik giriş korunacaktır.
        </p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-3xl p-6 text-slate-100 shadow-2xl">
            <div className="flex justify-between items-center pb-3 border-b border-slate-800">
              ...
              <button onClick={() => setIsForgotModalOpen(false)} className="p-1 text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleForgotPasswordSubmit} className="space-y-4 mt-4">
              <p className="text-xs text-slate-300">
                Kullanıcı kodunuzu veya kurumsal e-postanızı girdiğinizde şifre sıfırlama bağlantısı adresinize iletilecektir.
              </p>

              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">
                  Kullanıcı Kodu / E-Posta
                </label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="fatihsakar@orionarastirma.com"
